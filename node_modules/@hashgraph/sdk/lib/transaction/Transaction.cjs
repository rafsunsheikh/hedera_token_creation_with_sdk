...
class Transaction extends _Executable.default {
  ...
  static fromBytes(bytes) {
    ...
    for (const transaction of list) {
      ...
      if (transaction.bodyBytes && transaction.bodyBytes.length != 0) {
        ...
        const body = HashgraphProto.proto.TransactionBody.decode(transaction.bodyBytes);
    ...
    for (let index = 0; index < this._signedTransactions.length; index++) {
      ...
      if (signedTransaction.bodyBytes) {
        const {
          transactionID,
          nodeAccountID
        } = HashgraphProto.proto.TransactionBody.decode(signedTransaction.bodyBytes);
  ...
  async _makeRequestAsync() {
    ...
    if (!this._signOnDemand) {
      ...
      return /** @type {HashgraphProto.proto.ITransaction} */this._transactions.get(index);
    }
